        ...
        public static double DiscountedPrice(Coupon couponFromDb, double OriginalOrderTotal)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        //$10 off $100
                        OriginalOrderTotal = Math.Round(OriginalOrderTotal - couponFromDb.Discount, 2);
                    }
                    if (Convert.ToInt32(couponFromDb.CouponType) == (int)Coupon.ECouponType.Percent)
                    {
                        //10% off $100
                        OriginalOrderTotal = Math.Round(OriginalOrderTotal - (OriginalOrderTotal * couponFromDb.Discount / 100), 2);
                    }
                }
            }
            return OriginalOrderTotal;
